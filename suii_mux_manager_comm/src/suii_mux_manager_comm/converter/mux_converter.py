...
## ===== MuxConverter ===== ##
# Convert: ROS msg from MUX <-> ActionList()
class MuxConverter:
    @staticmethod 
    ...
    @staticmethod 
    def ros_to_task(ros_msg):
        task = Task()
        task.set_type(ros_msg.type)
        task.set_source(ros_msg.source)
        task.set_destination(ros_msg.destination)
        task.set_object(ros_msg.object)
        task.set_container(ros_msg.container)
    ...
    @staticmethod
    def task_to_ros(task, status):
        ros_task = SuiiTask()
        ros_task.type = task.type
        ros_task.source = task.source
        ros_task.destination = task.destination
        ros_task.object = task.object
        ros_task.container = task.container
    ...
    @staticmethod
    def task_list_to_ros(task_list, error_index=-1, status=-1):
        ros_task_list = SuiiTaskList()
        ros_task_list.error_index = error_index
        for item in task_list:
            ros_task_list.tasks.append(MuxConverter.task_to_ros(item, status))
        return ros_task_list